...
GKTL_FILE_HIJRIDATE=$TMPDIR/GeekTool_Hijri.txt
...
webURL=$BASEURL"&hanfishafi="$HANFISHAFI"&country="$COUNTRY"&state="$STATE"&state_name"$STATE_NAME"&latitude="$LATITUDE"&longitude="$LONGITUDE"&timezone="$TIMEZONE"&daylight="$DAYLIGHT"&pmethod="$PMETHOD
...
echo $xmlResult | sed -n -e 's/.*<hijri>\(.*\)<\/hijri>.*/\1/p' > $GKTL_FILE_HIJRIDATE
